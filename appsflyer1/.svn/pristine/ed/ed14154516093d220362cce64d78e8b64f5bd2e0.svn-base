...
<div id="ami_main">
	<div class="row-fluid" id="div1">	
			<form class="form-inline" action="log.do?method=retainedCountry&type=${params.type}" method="post" id="searchForm">
			...
			<thead>			
			 ...
			 <c:forEach items="${sum_daily}" var="one">
				...
				<tr id="tr_${user.id }" class="gradeA">
					...
					<td>${one.date}</td>
					<td>${one.ipCountries}</td> 
					<td>${one.channelType}</td>
					<td>${one.peopleNum}</td>
					<td><fmt:formatNumber type="number" value="${one.remain1 / one.peopleNum}" maxFractionDigits="2"/></td>
					<td><fmt:formatNumber type="number" value="${one.remain3 / one.peopleNum}" maxFractionDigits="2"/></td>
					<td><fmt:formatNumber type="number" value="${one.remain4 / one.peopleNum}" maxFractionDigits="2"/></td>
					<td><fmt:formatNumber type="number" value="${one.remain5 / one.peopleNum}" maxFractionDigits="2"/></td>
					<td><fmt:formatNumber type="number" value="${one.remain7 / one.peopleNum}" maxFractionDigits="2"/></td>
					<td><fmt:formatNumber type="number" value="${one.remain10 / one.peopleNum}" maxFractionDigits="2"/></td>
					<td><fmt:formatNumber type="number" value="${one.remain15 / one.peopleNum}" maxFractionDigits="2"/></td>
					<td><fmt:formatNumber type="number" value="${one.remain30 / one.peopleNum}" maxFractionDigits="2"/></td>
				</tr>
			  </c:forEach>
