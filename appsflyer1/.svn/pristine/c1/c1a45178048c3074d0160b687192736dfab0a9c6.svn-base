...
<form class="form-horizontal" action="compensation.do?method=doAddUserRefund&id=${one.id}" method="post" id="form" enctype="multipart/form-data">

	<h2 id="title" style="margin-left:50px;padding-bottom:30px">用户补偿</h2>

		
	<div class="control-group">
		<label class="control-label" for="form-field-accout">用户ID</label>
		<div class="controls">
			<input type="text" name="charId" value="${one.charId}"  class="validation" requiry="true">
		</div>
	</div>
...
$(document).ready(function () {
	...
	{ 
		...
		if(responseText == 0)
		{
		bootbox.alert("Compensation Information Saved",
			function() {
