...
<div id="ami_main">
<div class="row-fluid">	
			<form class="form-inline" action="game.do?method=queryFriendRequest" id="searchForm">
...
<div class="row-fluid">
		...
		<div class="ami_table_header"><span class="ami_table_header_title">好友申请</span>
			...
			<tbody>				
			 <c:forEach var="user" items="${PAGER.items}" varStatus="status">
				...
				<tr id="tr_${user.id }">
					...
					<td><a onclick="openNewWinow('player.do?method=loadByID&id=${user.id}')">${user.id}</a></td>
					<td>${user.charid}</td>
					<td>${user.sendid}</td>
