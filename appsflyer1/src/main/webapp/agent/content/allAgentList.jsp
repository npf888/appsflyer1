					...
					<tbody>
						<c:forEach items="${list}" var="agent">
							<tr id="tr_${agent.id}">
								...
								<td>
								<c:if test="${agent.bazooAgentDisplay == 0}">
									<button onclick="displayAgent(1,${agent.passportId})">显示</button>
								</c:if>
								<c:if test="${agent.bazooAgentDisplay == 1}">
									<button onclick="displayAgent(0,${agent.passportId})">不显示</button>
								</c:if>
...
function displayAgent(display,passportId){
	...
	bootbox.alert("保存成功",
			function() {
				nav('${pageContext.request.contextPath}/agentAll.do?method=all','全部代理商 ');
				});  
}
